...
static void _get(FILE *fp, void *buf, size_t memsize, size_t len, char *msg)
{
	...
	if (nread != len) {
		fprintf(stderr, "%s: need read %d, actually read %ld!\n", msg, len, nwrite);
		exit(0);
	}
}
...
void construct_gb(gb_info_t *gb, char *argv)
{
	...
	char file_bm_pos[GB_MAX_BUF];

	sscanf(argv, "%d %d %d %lf %s %s %s %s %s", &gb->type, &gb->numvis, &gb->numhid, &gb->lambda,
		file_vh, file_bm_pos, file_w);
	...
	get_data(file_bm_pos, gb->bm_pos, gb->numhid * gb->numhid * sizeof(uint8_t));
...
static void _out(FILE *fp, void *buf, size_t memsize, size_t len, char *msg)
{
	...
	if (nwrite != len) {
		fprintf(stderr, "%s: need write %d, actually write %ld!\n", msg, len, nwrite);
		exit(0);
	}
}
