...
//		w[xy] *= 0.01;
	...
	_numcase = train_numcase > dtr.numcase ? train_numcase : dtr.numcase;
//	_numcase = 50000;
	int *order = malloc(sizeof(int) * _numcase);
	if (order == NULL) {
		fprintf(stderr, "malloc error!\n");
		exit(0);
	}
/* // for train reorder
	randperm(order, 10);
	pr_array(stdout, order, 1, 10, 'i');
	pr_array(stdout, clssfy.w, 10, clssfy.numclass, 'd');
	reorder(clssfy.w, sizeof(double), order, 10, clssfy.numclass);
	fprintf(stdout, "\n");
	pr_array(stdout, clssfy.w, 10, clssfy.numclass, 'd');
	pr_array(stdout, dtr.labels, 1, 10, 'i');
	reorder(dtr.labels, sizeof(int), order, 10, 1);
	pr_array(stdout, dtr.labels, 1, 10, 'i');

	int test[] = {10, 10, 10, 10, 10};
	int ord[] = {0, 1, 2, 3, 4};
	reorder(test, sizeof(int), ord, 5, 1);
	pr_array(stdout, test, 1, 5, 'i');
...
*/
	...
	for (iter = 0; iter < train.iteration; iter++) {
		...
		if (curbatch >= totalbatch) {
			curbatch = 0;
		//	randperm(order, dtr.numcase);
		//	reorder(dtr.data, sizeof(uint8_t), order, dtr.numcase, case_info.numchannel * case_info.channelcase);
		//	reorder(dtr.labels, sizeof(int), order, dtr.numcase, 1);
		}
	}
