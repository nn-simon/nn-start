...
static void _get_data_from_H(double *data, const double *H, int *v2h, int len_v2h, int lencae, int numcase)
{
	int nc, nl;
...
double sm_train(sm_info_t *sm, const int *V, const double *H, int numcase, int batchsize)
{
	...
	double epsilonw = sm->learnrate;
	double epsilonb = sm->learnrate;
	...
	for (nb = 0; nb < total_batch; nb++) {
		cost = 0.0;
		//memset(softmax_gradw, 0, sm->class_max * sm->len_v2h_max * sizeof(double));
		const double *H_batch = H + nb * batchsize * numhid;
