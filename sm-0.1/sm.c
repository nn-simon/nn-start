...
static void _get_h2v(sm_info_t *sm)
{
	...
	int *count = (int *)malloc(sizeof(int) * sm->numhid);
	if (count == NULL) {
		fprintf(stderr, "malloc err in _get_h2v\n");
		exit(0);
	}
	memset(count, 0, sm->numhid * sizeof(int));
	...
	for (nv = 0; nv < sm->numvis; nv++) {
		for (nl = 0; nl < sm->len_v2h[nv]; nl++) {
			...
			sm->h2v[id_h][count[id_h]] = nv;
			sm->pos_h2v[id_h][count[id_h]] = nl;
			count[id_h]++;
		}
	}
	free(count);
